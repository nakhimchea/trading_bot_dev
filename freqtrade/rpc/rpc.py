...
class RPC:
    ...
    @staticmethod
    def _rpc_change_config(value: Dict) -> str:
        value = dict(value)
    ...
    @staticmethod
    def _rpc_newfile(value) -> str:
        value = dict(value)

        with open('variable.json', 'r') as fv:
            var = json.load(fv)
            port = var['port']

        directory = "configFiles/config{}.json".format(port)
        json_object = json.dumps(value, indent=4)
        ...
        with open("variable.json", "w") as wf:
            json.dump({'port': port}, wf)
